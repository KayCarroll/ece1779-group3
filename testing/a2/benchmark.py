...
def test_latency(num_requests = 1, request_type = "put"):
    if request_type == "put":
        ...
        total_response_time = 0
        list_of_items = get_list_of_keys(20)
        list_of_items = list_of_items*10
        random.shuffle(list_of_items)
        list_of_items = list_of_items[:num_requests]
        for item in list_of_items:
...
def constant_node_test(max_requests = 101, max_time = 25):
    ...
    requests.post(f"{API_URL}/configure_cache?mode=manual&numNodes=4&cacheSize=2&policy=LRU")
...
def varying_memcache_test(start_node=1, end_node=8, num_requests=20, num_seconds=5):
    ...
    if graph_type == "Growing":
        requests.post(API_URL + "/delete_all")
        test_latency(num_requests=50, request_type='put')
    x_axis = []
    latency_put_results = []
    latency_get_results = []
    test_done = False
...
def test():
    ...
    ###########################
    #constant_node_test(max_requests=30, max_time=25)
